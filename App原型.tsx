...
const AchievePage = (props: AchievePageProps) => (
  <div className='pageArea flexWrap'>
    {props.aiboRecord.map((val, ind) => val ? (
      <AiboCard key={ind} id={personaInfo[ind].id} star={personaInfo[ind].star} name={personaInfo[ind].name} />
    ) : (
      <AiboCard key={ind} id={personaInfo[ind].id} star={0} name={'未获得'} />
...
const QuestComponent = connect(
  ...
  const [isExploring, setIsExploring] = useState(props.userMapInfo[0][props.nowArea][props.questInfo.index].isExploring)
  const [isCompleted, setIsCompleted] = useState(props.userMapInfo[0][props.nowArea][props.questInfo.index].completeEasy)
  ...
  useInterval(
    () => {
      if (exploringProgress >= 100) {
        ...
        setIsCompleted(true)
        let userMapInfoTemp = props.userMapInfo
        userMapInfoTemp[0][props.nowArea][props.questInfo.index].isExploring = false
        userMapInfoTemp[0][props.nowArea][props.questInfo.index].completeEasy = true
        setUserMapInfo(userMapInfoTemp)
  ...
  function exploringStart () {
    ...
    if (!isExploring) {
      ...
      setIsExploring(true)
      let userMapInfoTemp = props.userMapInfo
      userMapInfoTemp[0][props.nowArea][props.questInfo.index].isExploring = true
      setUserMapInfo(userMapInfoTemp)
    }
  }
